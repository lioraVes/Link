...
const MultiPage: React.FC<MultiPageProps> = ({ title, icon, sub, checkboxHeader, checkboxes, buttons}) => {
      ...
      <div className={styles.checkboxContainer}>
        {checkboxes.map((option, index) => (
          <div key={index} className={styles.formCheckboxContainer}>
            <input type="checkbox" id={`checkbox-${index}`} checked={checkedState[index]}
              onChange={() => handleCheckboxChange(index)} className={styles.formCheckBox}
            />
            <label htmlFor={`checkbox-${index}`} className={styles.checkboxText}>
              {option}
            </label>
          </div>
        ))}
