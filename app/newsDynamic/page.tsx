...
const convertUrlsToLinks = (text: string) => {
  ...
  const websitePattern = /(?:www\.)?[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+(?:\/[^\s]*)?/g;

  // Replace URLs with anchor elements
  const parts = text.split(/\b(https?:\/\/[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+(?:\/[^\s]*)?|\b(?:www\.)?[a-zA-Z0-9-]+\.[a-zA-Z0-9-.]+(?:\/[^\s]*)?)/g);
  
  return parts.map((part, index) => {
    if (urlPattern.test(part) || websitePattern.test(part)) {
      // If part is a URL, return a link
      return (
        <a key={index} href={part.startsWith('http') ? part : `https://${part}`} target="_blank" rel="noopener noreferrer">
          {part}
        </a>
      );
    }
              ...
              <div className={styles.titleWithArrow}>
                ...
                {expandedIndex === i && (
                  <div className={styles.newsDateWrapper}>
                    ...
                    <span className={styles.publishedBy}>פורסם על ידי: מערכת האיגוד</span>
