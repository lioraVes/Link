...
import HandAnimation from "./Hand";

export default function HomePage() {
  const [openingFinished, setOpeningFinished] = useState(false); // Track if the animation is done
  const [handStopped, setHandStopped] = useState(false);
  ...
  const resumeHandAnimation = () => {
    setHandStopped(false);
    if (handAnimationRef.current) {
      handAnimationRef.current.play(); // Resume animation
    }
  };
  const stopHandAnimation = () => {
    setHandStopped(true);
    if (handAnimationRef.current) {
      handAnimationRef.current.goToAndStop(30, true); // Stop at frame 30
    }
  };
